...
//         cnLines : (int *)cnLines
{
	...
	for (int i = 0; i <= [indexArray count] - 1; i++) {
        ...
        //        NSLog(@"height1:%f height2:%f",engHight,[@"a" sizeWithFont:Courier constrainedToSize:CGSizeMake(textScroll.frame.size.width, CGFLOAT_MAX) lineBreakMode:UILineBreakModeWordWrap].height);
        cnHight = [@"赵" sizeWithFont:CourierTwo].height;
		int engLines = [[lyricArray objectAtIndex:i] sizeWithFont:Courier constrainedToSize:CGSizeMake(textScroll.frame.size.width, CGFLOAT_MAX) lineBreakMode:UILineBreakModeWordWrap].height / engHight;
        int cnLines = [[lyricCnArray objectAtIndex:i] length];
