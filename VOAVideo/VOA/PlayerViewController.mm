...
- (void)shareAll {
    NSString * Message = [NSString stringWithFormat:@"@爱语吧 VOA视频新闻的话题:%@是极好的,你也来听听吧,瞧瞧有木有",voa._title];
    ...
    if (nowUserID > 0) {
        ...
        [ShareToCNBox showWithText:Message link:[NSString stringWithFormat:@"http://apps.iyuba.com/voa/showVideo.jsp?voaId=%d&network=weibo&userId=%d",voa._voaid,nowUserID] titleId:voa._voaid];
    } else {
        [ShareToCNBox showWithText:Message link:[NSString stringWithFormat:@"http://apps.iyuba.com/voa/showVideo.jsp?voaId=%d&network=weibo",voa._voaid] titleId:voa._voaid];
    }
}

- (void)ShareThisQuestion{
    //    if (isExisitNet) { minutes
    NSString * url = Nil;
    //        url = [NSString stringWithFormat:@"http://apps.iyuba.com/voa/showItem.jsp?voaId=%d&network=weibo",voa._voaid];
    int nowUserID = [[[NSUserDefaults standardUserDefaults] objectForKey:@"nowUser"] integerValue];
    if (nowUserID > 0) {
        url = [NSString stringWithFormat:@"http://apps.iyuba.com/voa/showVideo.jsp?voaId=%d&network=weibo&userId=%d",voa._voaid,nowUserID];
    } else {
        url = [NSString stringWithFormat:@"http://apps.iyuba.com/voa/showVideo.jsp?voaId=%d&network=weibo",voa._voaid];
    }
